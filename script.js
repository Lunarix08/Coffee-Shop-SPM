...
function getCartItems() {
    const cartItems = [];
    const cartElements = document.querySelectorAll('.cart-items .cart-item'); // Adjust the selector based on your HTML structure

    cartElements.forEach(item => {
        const id = item.getAttribute('data-id'); // Assuming each cart item has a data-id attribute
        const name = item.querySelector('.item-name').innerText; // Adjust based on your HTML structure
        const quantity = item.querySelector('.item-quantity').value; // Assuming there's an input for quantity
        const price = item.querySelector('.item-price').innerText; // Adjust based on your HTML structure
...
document.getElementById('checkoutButton').addEventListener('click', function() {
    ...
    console.log('Cart Items:', cartItems); // Log the cart items to see if they are being captured correctly
    const idPelanggan = getCustomerId();
